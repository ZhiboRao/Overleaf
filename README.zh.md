...
- **原生菜单栏** 遵循 Mac 习惯：
  ...
  - `⌘,` 偏好设置
  - `⌘Q` 退出
  - `⌘F` 页面查找（Chrome 风格浮层；`⌘G` / `⇧⌘G` 上下翻、`Esc` 关闭）
...
- **下载面板**：与偏好设置共用同款"页式"窗框（大标题 + 副标题、SECTION 分组标签、分隔线、提示、底部按钮行），内部以 Motrix 风格卡片列出进行中与已完成下载，带彩色文件类型徽标、实时速度 / 剩余时间、进度条，以及 `Cancel`（同时把卡片从列表中移除）、`Retry`（中断时重试）、`Show in Finder` 按钮。超长文件名会在卡片内水平滚动，列表宽度始终不超过面板；下载完成后双击卡片即可在默认应用中打开文件。
