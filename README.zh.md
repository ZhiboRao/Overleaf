...
- **下载面板**：参考 Motrix 设计，浮动面板列出进行中与已完成下载，带彩色文件类型徽标、实时速度 / 剩余时间、进度条、`Cancel` 与 `Show in Finder` 按钮。
...
- **偏好设置**：iTerm2 风格的横向 Tab 布局；可修改首页地址、缩放比例、**语言**（`自动` / `English` → `www.overleaf.com` / `中文` → `cn.overleaf.com`）、下载目录，并切换通知 / Dock 徽标 / 钥匙串自动保存。
...
│   ├── downloads.py    # 参考 Motrix 的下载面板 / 下载卡片
